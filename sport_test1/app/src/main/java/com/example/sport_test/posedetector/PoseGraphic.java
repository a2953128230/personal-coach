package com.example.sport_test.posedetector;
...
public class PoseGraphic extends GraphicOverlay.Graphic {
    ...
    public PoseGraphic(
            ...
            List<String> poseClassification, boolean ff) {
        super(overlay);
        this.pose = pose;
    ...
    public void draw(Canvas canvas) {
        ...
        for (int i = 0; i < poseClassification.size(); i++) {
            ...
            // 如果 i == 0 且 poseClassification 的大小等於 2，則跳過本次循環
            if (i == 0 && poseClassification.size() == 2)
                continue;

            // 如果 ff 為 false，則跳過本次循環
            if (!ff)
                continue;
...
//        paint, canvas, visualizeZ, rescaleZForVisualization, avgZInImagePixel, zMin, zMax);
    ...
    void maybeUpdatePaintColor(Paint paint, Canvas canvas, float zInImagePixel) {
        if(!visualizeZ)
            return;

        // 當 visualizeZ 為 true 時，基於 z 值為不同的顏色設置畫筆。
        // 獲取 z 值範圍。
        float zLowerBoundInScreenPixel;
        float zUpperBoundInScreenPixel;

        if (rescaleZForVisualization) {
            // 如果需要重新調整 z 值範圍，則重新調整範圍到 [-0.001f, 0.001f] 之間。
            zLowerBoundInScreenPixel = Math.min(-0.001f, scale(zMin));
            zUpperBoundInScreenPixel = Math.max(0.001f, scale(zMax));
        } else {
            // 否則，假設 z 值範圍在 [-canvasWidth, canvasWidth] 之間。
        ...
        if (zInScreenPixel < 0) {
            // 如果當前 z 值在 z 軸原點前，則畫筆設置為紅色。
            // 將值從 [zLowerBoundInScreenPixel, 0) 映射到 [255, 0) 並用它來控制顏色。
            // 值越大，紅色越明顯。
            int v = (int) (zInScreenPixel / zLowerBoundInScreenPixel * 255);
            v = Ints.constrainToRange(v, 0, 255);
            paint.setARGB(255, 255, 255 - v, 255 - v);
        } else {
            // 如果當前 z 值在 z 軸原點後，則畫筆設置為藍色。
            // 將值從 [0, zUpperBoundInScreenPixel] 映射到 [0, 255] 並用它來控制顏色。
            // 值越大，藍色越明顯。
